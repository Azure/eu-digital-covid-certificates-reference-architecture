# Create a UAMI
resource "azurerm_user_assigned_identity" "dgca_issuance_service_identity" {
...
resource "azurerm_role_assignment" "dgca_issuance_service_kv_role_assignment" {
  ...
  principal_id         = azurerm_user_assigned_identity.dgca_issuance_service_identity.principal_id
  scope                = each.value.scope
  role_definition_name = each.value.role_definition_name
}

# Database
resource "azurerm_mysql_database" "issuance_service_db" {
...
# Namespace
resource "kubernetes_namespace" "dgca_issuance_service" {
  metadata {
    name = "dgca-issuance-service"
  }
...
resource "helm_release" "dgca_issuance_service" {
  name              = "dgca-issuance-service"
  chart             = "../charts/dgca-issuance-service"
  namespace         = "dgca-issuance-service"
    ...
    jsonencode({
      ...
      "spring" = {
        "datasource" = {
          ...
          "username" : "${azurerm_user_assigned_identity.dgca_issuance_service_identity.name}@${module.base_infra.mysql_server_name}"
        }
      }
      ...
      "aadPodIdentity" = {
        "enabled" : true
        "resourceID" : azurerm_user_assigned_identity.dgca_issuance_service_identity.id
        "clientID" : azurerm_user_assigned_identity.dgca_issuance_service_identity.client_id
      }
    ...
    jsonencode({
      "mysql-aad-setup" = {
        ...
        "database" = {
          ...
          "user"      = azurerm_user_assigned_identity.dgca_issuance_service_identity.name
          "clientID"  = azurerm_user_assigned_identity.dgca_issuance_service_identity.client_id
        }
  ...
  depends_on = [
    module.base_infra,
    azurerm_mysql_database.issuance_service_db,
    kubernetes_namespace.dgca_issuance_service,
    azurerm_role_assignment.dgca_issuance_service_kv_role_assignment,
  ]
}
