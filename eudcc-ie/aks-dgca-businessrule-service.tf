# Create a UAMI
resource "azurerm_user_assigned_identity" "dgca_businessrule_service_identity" {
...
resource "azurerm_role_assignment" "dgca_businessrule_service_kv_role_assignment" {
  ...
  principal_id         = azurerm_user_assigned_identity.dgca_businessrule_service_identity.principal_id
  scope                = each.value.scope
  role_definition_name = each.value.role_definition_name
}

# Database
resource "azurerm_mysql_database" "businessrule_service_db" {
...
# Namespace
resource "kubernetes_namespace" "dgca_businessrule_service" {
  metadata {
    name = "dgca-businessrule-service"
  }
...
# Deploy DGCA Business Rule Service
resource "helm_release" "dgca_businessrule_service" {
  name              = "dgca-businessrule-service"
  chart             = "../charts/dgca-businessrule-service"
  namespace         = "dgca-businessrule-service"
  ...
  values = [
    jsonencode({
      "ingress" = {
        ...
        "hosts" = [{
          "host" = "dgca-businessrule-service.${module.base_infra.dns_zone_name}"
          "paths" = [{
            "path"     = "/"
            "pathType" = "Prefix"
          }]
        ...
        "tls" = [{
          "secretName" = "dgca-businessrule-service-public-tls"
          "hosts" = [
            "dgca-businessrule-service.${module.base_infra.dns_zone_name}"
          ]
        }]
      }

      "image" = {
        "repository" = "${module.base_infra.acr_login_server}/eu-digital-covid-certificates/dgca-businessrule-service"
        "tag"        = var.businessrule_service_version
      }
      ...
      "spring" = {
        "datasource" = {
          "url" : "jdbc:mysql://${module.base_infra.mysql_server_fqdn}:3306/${azurerm_mysql_database.businessrule_service_db.name}?useSSL=true&requireSSL=true"
          "username" : "${azurerm_user_assigned_identity.dgca_businessrule_service_identity.name}@${module.base_infra.mysql_server_name}"
        }
      }
      ...
      "aadPodIdentity" = {
        "enabled" : true
        "resourceID" : azurerm_user_assigned_identity.dgca_businessrule_service_identity.id
        "clientID" : azurerm_user_assigned_identity.dgca_businessrule_service_identity.client_id
      }
    ...
    jsonencode({
      "mysql-aad-setup" = {
        ...
        "database" = {
          ...
          "name"      = azurerm_mysql_database.businessrule_service_db.name
          "adminUser" = "${module.base_infra.mysql_aadadmin_identity_name}@${module.base_infra.mysql_server_name}"
          "user"      = azurerm_user_assigned_identity.dgca_businessrule_service_identity.name
          "clientID"  = azurerm_user_assigned_identity.dgca_businessrule_service_identity.client_id
        }
  ...
  depends_on = [
    ...
    kubernetes_namespace.dgca_businessrule_service,
    azurerm_role_assignment.dgca_businessrule_service_kv_role_assignment,
  ]
}
